...
export function useUnverifiedAgents() {
  return useQuery({
    ...
    queryFn: async () => {
      const res = await fetch(api.users.listUnverifiedAgents.path);
...
export function useCreditAgent() {
  ...
  return useMutation({
    mutationFn: async ({ id, amount }: { id: string; amount: number }) => {
      const res = await fetch(`/api/admin/wallet/${id}/load`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ amount }) });
