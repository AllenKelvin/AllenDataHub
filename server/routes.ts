      ...
      const list = await Promise.all(
        orders.map(async (order: any) => {
          ...
          return {
            ...
            clientOrderReference: order.clientOrderReference || null,
            productName: order.productName || product?.name || "Unknown",
            productNetwork: product?.network || "",
            dataAmount: order.dataAmount || product?.dataAmount,
            createdAt: order.createdAt,
    ...
    try {
      ...
      res.status(201).json({
        order: {
          ...
          price: perItemPrice,
          orderSource: "api",
          vendorOrderId: finalOrder?.vendorOrderId || null,
          clientOrderReference: finalOrder?.clientOrderReference || null,
          walletBalanceBefore: balanceBefore,
          walletBalanceAfter: balanceAfter,
