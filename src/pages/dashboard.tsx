              ...
              <div className="overflow-x-auto pb-1">
                <div className="flex min-w-max gap-3">
                  {recentOrders.map((order) => (
                    <div key={order.id} className="min-w-[220px] rounded-2xl border border-slate-200 bg-slate-50/80 p-3 dark:border-slate-700 dark:bg-slate-800/80">
                      <div className="flex items-center justify-between gap-2">
                        <p className="truncate text-xs font-semibold uppercase tracking-wide text-slate-500 dark:text-slate-400">#{order.id}</p>
                        <StatusBadge status={order.status} />
                      </div>
                      <div className="mt-3 space-y-1">
                        <p className="text-sm font-semibold text-slate-800 dark:text-slate-100">{order.network}</p>
                        <p className="text-xs text-slate-500 dark:text-slate-400">{order.recipient}</p>
              ...
              <div className="overflow-x-auto pb-1">
                <div className="flex min-w-max gap-3">
                  {dashboard.recentDeposits.slice(0, 5).map((d) => (
                    <div key={d.id} className="min-w-[220px] rounded-2xl border border-slate-200 bg-emerald-50/80 p-3 dark:border-slate-700 dark:bg-emerald-950/20">
                      <div className="flex items-center justify-between gap-2">
                        <p className="text-xs font-semibold uppercase tracking-wide text-emerald-700 dark:text-emerald-300">Deposit</p>
                        <StatusBadge status={d.status.toLowerCase()} />
                      </div>
                      <p className="mt-3 text-lg font-bold text-emerald-600 dark:text-emerald-300">{formatGHS(d.amount).replace("GHS ", "GHS")}</p>
                      <p className="mt-1 text-xs text-slate-500 dark:text-slate-400">{d.date}</p>
                    </div>
                  ))}
                </div>
              </div>
